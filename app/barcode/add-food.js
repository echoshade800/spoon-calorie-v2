/**
 * Barcode Add Food Screen - Add matched food from barcode scan
 * 
 * Purpose: Display matched food with barcode info and allow portion adjustment
 * Features: Barcode match indicator, portion adjustment, meal selection, save to diary
 */
...
export default function BarcodeAddFoodScreen() {
  ...
  const loadFoodData = async () => {
    try {
      ...
      const mockFood = {
        id: params.foodId,
        name: params.foodName || 'Coca-Cola Classic',
        brand: 'The Coca-Cola Company',
        kcal_per_100g: 42,
        carbs_per_100g: 10.6,
        protein_per_100g: 0,
        fat_per_100g: 0,
        serving_label: '1 can (330ml)',
      ...
      // Set default serving
      const defaultServing = getDefaultServing(mockFood);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Success',
        `Logged to ${getMealDisplayName(selectedMeal)}`,
  ...
  const handleFindBetterMatch = () => {
    router.push({
      pathname: '/barcode/search',
      params: {
  ...
  const renderBarcodeMatchBanner = () => (
    <View style={styles.matchBanner}>
      <Text style={styles.matchText}>
        This barcode was matched to: "{food?.name}"
      </Text>
      <TouchableOpacity onPress={handleFindBetterMatch}>
        <Text style={styles.matchLink}>Find a better match</Text>
      </TouchableOpacity>
    </View>
  );
          ...
          <View style={styles.nutritionContent}>
            <View style={styles.caloriesContainer}>
              <View style={styles.caloriesCircle}>
                <Text style={styles.caloriesNumber}>{nutrition.kcal}</Text>
